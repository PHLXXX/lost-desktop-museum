...
export function PhotosApp() {
  const [index, setIndex] = useState(0); const [metadata, setMetadata] = useState(false); const [zoom, setZoom] = useState(1)
  const photo = caseDefinition.photos[index]!; const investigate = useGameStore((state) => state.investigate)
  return <div className="photo-app"><div className="photo-stage"><img src={photo.image} alt="虚构海津机场候机区" style={{ transform: `scale(${zoom})` }} /></div><aside><p className="document-meta">{photo.title}</p><div className="toolbar"><button onClick={() => { setIndex((index - 1 + 2) % 2); setMetadata(false) }}>上一张</button><button onClick={() => { setIndex((index + 1) % 2); setMetadata(false) }}>下一张</button><button onClick={() => setZoom(Math.max(.7, zoom - .15))}>−</button><button onClick={() => setZoom(Math.min(1.8, zoom + .15))}>＋</button></div><button className="primary-button" onClick={() => { setMetadata(true); investigate({ type: 'VIEW_METADATA', itemId: photo.id }) }}>查看元数据</button>{metadata && <dl className="metadata"><dt>原始拍摄</dt><dd>{photo.metadata.capturedAt}</dd><dt>导出文件</dt><dd>{photo.metadata.exportedAt}</dd><dt>设备</dt><dd>{photo.metadata.camera}</dd></dl>}</aside></div>
}
...
export function RecycleApp() {
  const unlocked = useGameStore((state) => state.unlockedItemIds); const investigate = useGameStore((state) => state.investigate); const [selected, setSelected] = useState<string | null>(null)
  const files = caseDefinition.files.filter((file) => file.folder === '回收站' && (!file.locked || unlocked.includes(file.id)))
  const active = files.find((file) => file.id === selected)
  return <div className="split-app two-column"><section className="app-list">{files.map((file) => <button key={file.id} onClick={() => { setSelected(file.id); if (file.id === 'farewell-v3') investigate({ type: 'COMPARE_ITEMS', itemId: file.id }) }}><span><strong>{file.name}</strong><small>可预览 · 可恢复</small></span></button>)}</section><article className="detail-pane document-view">{active ? <><div className="document-meta">回收站预览</div><h3>{active.name}</h3><p>{active.content}</p><button className="secondary-button">恢复到原位置</button></> : <div className="empty-state">比较三个版本，看看“离开”的对象如何变化。</div>}</article></div>
}

export function LogsApp() {
  const [user, setUser] = useState('全部'); const [type, setType] = useState('全部'); const investigate = useGameStore((state) => state.investigate)
  const logs = caseDefinition.logs.filter((log) => (user === '全部' || log.user === user) && (type === '全部' || log.eventType === type))
  return <div className="single-app logs-app"><div className="app-toolbar"><select aria-label="用户筛选" value={user} onChange={(event) => setUser(event.target.value)}>{['全部', 'ZHOU_YU', 'LINRAN', 'SYSTEM'].map((item) => <option key={item}>{item}</option>)}</select><select aria-label="事件类型筛选" value={type} onChange={(event) => setType(event.target.value)}>{['全部', '登录', '账户', '文件', '异常'].map((item) => <option key={item}>{item}</option>)}</select></div><div className="log-table"><div className="log-head"><span>时间</span><span>用户</span><span>类型</span><span>事件</span></div>{logs.map((log) => <button key={log.id} onClick={() => investigate({ type: 'VIEW_LOG', itemId: log.id })}><time>{log.time}</time><b>{log.user}</b><em>{log.eventType}</em><span>{log.detail}</span></button>)}</div></div>
}
