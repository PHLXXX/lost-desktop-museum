...
export function HistoryApp() {
  ...
  const entries = useMemo(() => caseDefinition.browser.filter((entry) => {
    ...
    return (date === '全部日期' || entry.time.startsWith(date)) && haystack.includes(query.toLowerCase())
  }).sort((a, b) => descending ? b.time.localeCompare(a.time) : a.time.localeCompare(b.time)), [caseDefinition.browser, query, date, descending])
  const dates = [...new Set(caseDefinition.browser.flatMap((entry) => /^(\d{4}-\d{2}-\d{2})/.exec(entry.time)?.[1] ?? []))]
  ...
  return (
    <div className="application history-app">
      <AppToolbar>
        ...
        <select aria-label="日期筛选" value={date} onChange={(event) => setDate(event.target.value)}><option>全部日期</option>{dates.map((item) => <option key={item}>{item}</option>)}</select>
      ...
      <div className="history-layout">
        ...
        <aside className="record-detail"><PaneHeader title="记录详情" />{selected ? <><dl><dt>标题</dt><dd>{selected.title}</dd><dt>访问时间</dt><dd>{selected.time}</dd><dt>域名</dt><dd>{domain(selected.category)}</dd><dt>访问次数</dt><dd>1</dd><dt>设备</dt><dd>本机档案终端</dd><dt>恢复方式</dt><dd>本地 History 数据库</dd></dl><button disabled={!clueId || !discoveredClueIds.includes(clueId)} onClick={() => clueId && togglePinned(clueId)}>{clueId && pinnedClueIds.includes(clueId) ? '移出证据板' : '加入证据板'}</button></> : <div className="empty-state">选择记录以查看详情</div>}</aside>
      </div>
      <AppStatusBar><span>{entries.length} 条历史记录</span><span>快照时间 {caseDefinition.desktop.lastLoginMessage}</span></AppStatusBar>
    </div>
  )
}
