...
function modifiedAt(file: VirtualFile, definition: CaseDefinition) { return file.modifiedAt ?? definition.clues.find((clue) => clue.discovery.itemId === file.id)?.times[0] ?? '未记录' }
...
export function FilesApp() {
  const caseDefinition = useActiveCaseDefinition()
  const { investigate, openIdentityDraft, restoredItemIds, unlockedItemIds, discoveredClueIds, pinnedClueIds, togglePinned } = useGameStore()
  ...
  const fileProgress = useMemo(() => ({ restoredItemIds, unlockedItemIds }), [restoredItemIds, unlockedItemIds])
  const filesWithRestores = useMemo(() => filesInFolder(caseDefinition, folder, fileProgress), [caseDefinition, fileProgress, folder])
  const visible = useMemo(() => filesWithRestores.filter((file) => file.name.toLowerCase().includes(query.toLowerCase())).sort((a, b) => {
    const comparison = sort === 'name' ? a.name.localeCompare(b.name) : modifiedAt(a, caseDefinition).localeCompare(modifiedAt(b, caseDefinition))
    return descending ? -comparison : comparison
  }), [caseDefinition, filesWithRestores, query, sort, descending])
