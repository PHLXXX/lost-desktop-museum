...
import { caseSnapshotDate, chineseDate } from '../../cases/casePresentation'
...
export function MessagesApp() {
  ...
  const [thread, setThread] = useState(caseDefinition.chats[0] ?? null)
  ...
  if (!activeThread) {
    return <div className="application messages-app"><section className="empty-state app-empty-state"><h2>没有恢复到会话</h2><p>当前案件包没有提供消息记录，其他调查应用仍可正常使用。</p></section><AppStatusBar><span>0 个会话</span><span>本地档案为空</span></AppStatusBar></div>
  }
  ...
  const snapshotDate = chineseDate(caseSnapshotDate(caseDefinition))

  return (
    <div className="application messages-app">
      <AppToolbar><strong>讯息存档</strong><input type="search" aria-label="搜索会话" placeholder="搜索联系人或消息" value={query} onChange={(event) => setQuery(event.target.value)} /><span>只读快照 · {snapshotDate}</span></AppToolbar>
      <div className="messages-layout">
        ...
        <section className="message-timeline"><PaneHeader title={activeThread.title} meta="本地消息存档 · 离线" /><div className="timeline-date">{snapshotDate}</div>{activeThread.messages.map((message) => <article className={`message-entry ${message.sender === caseDefinition.subject.name ? 'mine' : ''}`} key={message.id}><button className="message-body" onClick={() => investigate({ type: 'OPEN_ITEM', itemId: message.id })}><span className="message-author">{message.sender} · {message.time}</span><p>{message.text}</p><small>{message.sender === caseDefinition.subject.name ? '已读' : message.unread ? '未读' : '已接收'}</small></button>{message.attachmentId && <button className="message-attachment" aria-label={`打开图片附件 ${message.attachmentId}`} onClick={() => { useApplicationStore.getState().selectPhoto(message.attachmentId!); useWindowStore.getState().openWindow('photos') }}><span className="attachment-placeholder">{message.attachmentId}</span><small>图片附件 · 在照片中打开</small></button>}</article>)}</section>
        <aside className="conversation-info"><PaneHeader title="会话详情" /><dl><dt>参与者</dt><dd>{caseDefinition.subject.name}、{activeThread.title}</dd><dt>在线状态</dt><dd>离线存档</dd><dt>消息数量</dt><dd>{activeThread.messages.length}</dd><dt>首条记录</dt><dd>{activeThread.messages[0]?.time}</dd><dt>数据来源</dt><dd>本地消息数据库快照</dd></dl><button disabled={!threadClues.length} onClick={() => threadClues[0] && togglePinned(threadClues[0])}>{threadClues[0] && pinnedClueIds.includes(threadClues[0]) ? '移出证据板' : '将已发现消息加入证据板'}</button></aside>
      </div>
