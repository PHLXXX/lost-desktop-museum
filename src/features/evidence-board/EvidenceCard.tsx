...
export function EvidenceCard({ clue, pinned, position, onToggle, onMove, onSelect }: { clue: ClueDefinition; pinned: boolean; position: { x: number; y: number }; onToggle: () => void; onMove: (x: number, y: number) => void; onSelect?: () => void }) {
  ...
  return <article className={`evidence-card ${pinned ? 'pinned' : ''}`} style={{ left: position.x, top: position.y }} onClick={onSelect}>
