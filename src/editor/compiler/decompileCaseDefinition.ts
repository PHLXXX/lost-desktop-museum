...
export function decompileCaseDefinition(definition: CaseDefinition): CaseDraft {
  return {
    ...
    audioTracks: structuredClone(definition.audioTracks), broadcastEvents: structuredClone(definition.broadcastEvents), dataTables: structuredClone(definition.dataTables), terminalEntries: structuredClone(definition.terminalEntries), versionDiffs: structuredClone(definition.versionDiffs), sitemap: structuredClone(definition.sitemap), clues: structuredClone(definition.clues), triggers: structuredClone(definition.triggers),
    gameplay: definition.gameplay ? structuredClone(definition.gameplay) : undefined,
    deduction: { questions: structuredClone(definition.questions), coreEvidenceIds: structuredClone(definition.coreEvidenceIds), correctContradictions: structuredClone(definition.correctContradictions), ending: definition.ending, resultLevels: structuredClone(definition.resultLevels) }, assets: structuredClone(definition.assets),
  }
}
