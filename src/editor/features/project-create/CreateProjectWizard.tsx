...
export function CreateProjectWizard({ onClose }: { onClose: () => void }) {
  ...
  const [details, setDetails] = useState({ name: '未命名案件工程', caseId: 'case-workshop-demo', title: '消失的备用钥匙', author: 'independent-author' })
  ...
  return <div className="workshop-modal-backdrop"><section className="workshop-modal" role="dialog" aria-modal="true" aria-labelledby="create-project-title">
    ...
    <div className="wizard-body">
      ...
      {step === 2 && <div className="editor-form"><label>工程名称<input value={details.name} onChange={(event) => setDetails({ ...details, name: event.target.value })} /></label><label>caseId<input value={details.caseId} pattern="[a-z0-9-]+" onChange={(event) => setDetails({ ...details, caseId: event.target.value })} /></label><label>案件名称<input value={details.title} onChange={(event) => setDetails({ ...details, title: event.target.value })} /></label><label>发布者 ID<input value={details.author} pattern="[a-z0-9-]{3,40}" onChange={(event) => setDetails({ ...details, author: event.target.value })} /><small>社区投稿使用的小写稳定 ID，例如 independent-author。</small></label><label>语言<select defaultValue="zh-CN"><option>zh-CN</option></select></label><label>版本<input defaultValue="1.0.0" /></label></div>}
