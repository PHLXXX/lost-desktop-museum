...
describe('community prepare CLI core', () => {
  it('validates in dry-run mode without writing output', async () => {
    ...
    expect(result.entry.caseId).toBe('case-community-sample-001'); expect(result.entry.engineCompatibility.minimum).toBe('0.6.0'); await expect(access(output)).rejects.toThrow()
  })
})
