...
export async function validateAssetFile(file: Pick<File, 'name' | 'type' | 'slice'>) {
  ...
  if (!allowedMimes) return { valid: false, message: `${file.name} 的扩展名不在安全白名单中。` }
  if (!allowedMimes.includes(file.type.toLowerCase())) return { valid: false, message: `${file.name} 的MIME类型与扩展名不匹配。` }
  const bytes = new Uint8Array(await file.slice(0, 4096).arrayBuffer())
  if (!matchesSignature(extension, bytes)) return { valid: false, message: `${file.name} 的文件签名与声明类型不匹配。` }
