...
export function EditorTopbar({ onReturnMuseum, onPreview, onPublish, onSnapshots, readOnly = false }: { onReturnMuseum: () => void; onPreview: () => void; onPublish: () => void; onSnapshots: () => void; readOnly?: boolean }) {
  ...
  return <header className="editor-topbar">
    ...
    <div className="editor-toolbar" role="toolbar" aria-label="工程工具栏">
      <button aria-label="返回工程列表" onClick={() => void leave()}>工程列表</button><button onClick={() => void leave(onReturnMuseum)}>档案馆</button><i />
      <button disabled={readOnly} onClick={undo} aria-label="撤销">↶</button><button disabled={readOnly} onClick={redo} aria-label="重做">↷</button><button disabled={readOnly} onClick={() => void saveNow()}>立即保存</button><button disabled={readOnly} onClick={onSnapshots}>快照</button>
      <button onClick={() => setSearching(!searching)}>全局搜索</button><button aria-label="运行校验" onClick={() => { void validate(); setSection('validation') }}>校验</button><button onClick={onPreview}>试玩</button><button className="editor-export" disabled={readOnly} onClick={onPublish}>导出</button>
    </div>
